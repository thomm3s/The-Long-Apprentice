...
Prototype scaffolding exists and loads cleanly: `project.godot`, `scenes/player/Player.tscn`, `scenes/world/Main.tscn`, `scripts/Player.gd` (basic `CharacterBody3D` movement + raycast interact), `scenes/props/Tree.tscn`, `scripts/Tree.gd` (chop-able placeholder tree), `scripts/Inventory.gd` (autoload singleton, numbers-only item counts), `scenes/ui/HUD.tscn` + `scripts/HUD.gd` (CanvasLayer wood-count label, live-updates off `Inventory.changed`) — all verified via headless import + instantiate on Godot 4.7, no errors. Project upgraded from 4.3 to 4.7 on 2026-07-10 (config/features auto-bumped by the engine on import). Player can walk up to a Tree instance in Main.tscn and press E to chop it (removes tree, calls `Inventory.add("wood", 1)`, now reflected live in the on-screen HUD label).
